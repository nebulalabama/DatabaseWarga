...
@section('title', 'Data Keluarga - Dbwarga')
...
<div class="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-6">
    <div class="flex items-center justify-between my-4">
        ...
        <a href="{{ route('family.create') }}" class="inline-flex items-center px-4 py-2 font-bold text-white bg-green-500 border-b-4 border-green-700 rounded hover:bg-green-400 hover:border-green-500">
        ...
        <table class="min-w-full border-collapse">
            <thead class="bg-gray dark:bg-meta-4">
                <tr>
                    ...
                    <th class="p-2.5 xl:p-5 text-sm font-medium uppercase xsm:text-base">No Kartu Keluarga</th>
                    <th class="p-2.5 text-center xl:p-5 text-sm font-medium uppercase xsm:text-base">Kepala Keluarga</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-stroke dark:border-strokedark">
                    ...
                    <td class="p-2.5 text-center xl:p-5 text-sm xsm:text-base">{{ $familyCard->no_kk }}</td>
                    <td class="p-2.5 text-center xl:p-5 text-sm xsm:text-base">{{ $familyCard->resident->name }}</td>
                    <td class="flex justify-center items-center p-2.5 xl:p-5">
                        <a href="{{ route('family.show', $familyCard->id) }}" class="mr-2 text-grey hover:text-green-600 hover:underline">
                        ...
                        <a href="{{ route('family.edit', $familyCard->id) }}" class="mr-2 text-grey hover:text-blue-600 hover:underline">
                            <svg xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em" viewBox="0 0 24 24" class="w-6 h-6 sm:w-8 sm:h-8">
                                <g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">
                        ...
                        <form action="{{ route('family.destroy', $familyCard->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
