    ...
    <div
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('users.create') }}"
                class="inline-flex items-center px-4 py-2 font-bold text-white bg-green-500 border-b-4 border-green-700 rounded hover:bg-green-400 hover:border-green-500">
            ...
            <table class="min-w-full border-collapse">
                <thead class="bg-gray dark:bg-meta-4">
                    <tr>
                        ...
                        <th class="p-2.5 xl:p-5 text-sm font-medium uppercase xsm:text-base">Nama</th>
                        <th class="p-2.5 text-center xl:p-5 text-sm font-medium uppercase xsm:text-base">Email</th>
                        <th class="p-2.5 text-center xl:p-5 text-sm font-medium uppercase xsm:text-base">Role</th>
                        <th class="p-2.5 text-center xl:p-5 text-sm font-medium uppercase xsm:text-base">Phone</th>
                        <th class="p-2.5 text-center xl:p-5 text-sm font-medium uppercase xsm:text-base">Status</th>
                ...
                <tbody>
                    @foreach ($users as $user)
                        <tr class="border-b border-stroke dark:border-strokedark">
                            <td class="p-2.5 xl:p-5 text-sm xsm:text-base">{{ $loop->iteration }}</td>
                            <td class="p-2.5 xl:p-5 text-sm xsm:text-base">{{ $user->name }}</td>
                            <td class="p-2.5 text-center xl:p-5 text-sm xsm:text-base">{{ $user->email }}</td>
                            <td class="p-2.5 text-center xl:p-5 text-sm xsm:text-base">{{ $user->role }}</td>
                            <td class="p-2.5 text-center xl:p-5 text-sm xsm:text-base">{{ $user->phone }}</td>
                            ...
                            <td class="flex justify-center items-center p-2.5 xl:p-5">
                                <a href="{{ route('users.show', ['id' => $user->id]) }}"
                                    class="mr-2 text-grey hover:text-green-600 hover:underline">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em"
                                        viewBox="0 0 24 24" class="w-6 h-6 sm:w-8 sm:h-8">
                                ...
                                <a href="{{ route('users.edit', ['id' => $user->id]) }}"
                                    class="mr-2 text-grey hover:text-blue-600 hover:underline">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em"
                                        viewBox="0 0 24 24" class="w-6 h-6 sm:w-8 sm:h-8">
                                        <g fill="none" stroke="currentColor" stroke-linecap="round"
                                            stroke-linejoin="round" stroke-width="2">
