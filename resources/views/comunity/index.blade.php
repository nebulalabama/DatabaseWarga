...
@section('content')
    ...
    <div
        ...
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('community_units.create') }}"
            ...
            <table class="min-w-full bg-white dark:bg-meta-4">
                <thead>
                    <tr class="w-full bg-gray dark:bg-meta-4">
                        <th class="p-2.5 xl:p-5 text-sm font-medium uppercase xsm:text-base">RW</th>
                        <th class="p-2.5 xl:p-5 text-sm font-medium uppercase xsm:text-base text-center">Nama Ketua RW</th>
                        <th class="p-2.5 xl:p-5 text-sm font-medium uppercase xsm:text-base text-center">Action</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($data as $item)
					<tr class="border-b border-stroke dark:border-strokedark dark:bg-boxdark">
                        <td class="p-2.5 xl:p-5 text-sm xsm:text-base">{{ $item->name }}</td>
                        <td class="p-2.5 xl:p-5 text-sm xsm:text-base text-center">{{ $item->head }}</td>
                        <td class="p-2.5 xl:p-5 flex justify-center items-center">
                            <a href="{{ route('community_units.show', $item->id) }}" class="mr-2 text-grey hover:text-green-600 hover:underline">
                            ...
                            <a href="{{ route('community_units.edit', $item->id) }}" class="mr-2 text-grey hover:text-blue-600 hover:underline">
                                <svg xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em" viewBox="0 0 24 24"
                                    class="w-6 h-6 sm:w-8 sm:h-8">
                                    <g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                        stroke-width="2">
                            ...
                            <form action="{{ route('community_units.destroy', $item->id) }}" method="post">
