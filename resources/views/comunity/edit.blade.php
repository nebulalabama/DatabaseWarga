...
@section('title', 'Ubah RW - Dbwarga')

@section('content')
<div class="max-w-md mx-auto">
    <div class="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <h1 class="mb-6 text-2xl font-bold text-center">Ubah RW</h1>

        <form action="{{ route('community_units.update', $data->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="no_rw" class="block mb-2 text-sm font-bold text-gray-700">Nomor RW</label>
                <div class="flex items-center border rounded-md">
                    ...
                    <input class="w-full px-4 py-2 outline-none dark:bg-meta-4" value="{{ $data->name }}" id="no_rw" name="no_rw" type="text" placeholder="contoh: 001, 005,...">
                </div>
            </div>

            <div class="mb-6">
                <label for="head" class="block mb-2 text-sm font-bold text-gray-700">Nama Ketua RW</label>
                <div class="flex items-center border rounded-md">
                    <span class="px-3 text-gray-700 bg-gray-200 ">
                    ...
                    <input class="w-full px-4 py-2 outline-none dark:bg-meta-4" value="{{ $data->head }}" id="head" name="head" type="text" placeholder="Nama Ketua RW">
                </div>
            </div>
